    ...
    return (
        <div className="qual-row">
            ...
            <div className="qual-node">
                ...
                >
                    <img src={item.logo} alt={item.subtitle} className="qual-badge__img" style={{ '--qual-logo-zoom': item.logoZoom ?? 1.15 }} />
                </motion.div>
                {!isLast && (
                    <motion.div
...
const TimelineList = ({ items, faceKey }) => (
    <div className="qual-timeline">
        {items.map((item, i) => (
            <TimelineRow key={`${faceKey}-${i}`} item={item} idx={i} isLast={i === items.length - 1} />
        ))}
    </div>
);
        ...
        <MotionSection className="work" id="work" gap={0.12}>
            ...
            <div className="qualification__container container">
                <motion.div className="qualification__tabs" variants={fadeUp}>
                    <button className={`qualification__button ${tab === 1 ? 'qualification__active' : ''}`} onClick={() => setTab(1)} type="button">
                        {tab === 1 && <motion.span className="qualification__tab-pill" layoutId="qual-tab-pill" transition={{ type: 'spring', stiffness: 380, damping: 30 }} />}
                        <span className="qualification__tab-label">
                            <i className="uil uil-briefcase-alt qualification__icon"></i> Work Experience
                        </span>
                    </button>
                    <button className={`qualification__button ${tab === 2 ? 'qualification__active' : ''}`} onClick={() => setTab(2)} type="button">
                        {tab === 2 && <motion.span className="qualification__tab-pill" layoutId="qual-tab-pill" transition={{ type: 'spring', stiffness: 380, damping: 30 }} />}
                        <span className="qualification__tab-label">
                ...
                <div className="qualification__sections">
                    {reduceMotion ? (
                        ...
                        <div className="qual-flip-scene">
                            <motion.div className="qual-flip-card" animate={{ rotateY: tab === 1 ? 0 : 180 }} transition={{ duration: FLIP_DURATION, ease: [0.45, 0.05, 0.55, 0.95] }}>
